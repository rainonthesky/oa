    ...
    var roleMgr = { 		
   		delRole:function(roleId){	
 	 		if(confirm("您确定要删除此角色吗?")){  	 			 
	 		  $.ajax({
					type:'post',//请求方式
					url:'${ctx}/sys/role/delRole.action',
...
<body>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/sys/role/gotoRoleEdit?editFlag=1">角色添加</a></li>
	</ul>

	<form id="roleListForm" method="post" action="${ctx}/sys/role/gotoRoleList.action">
		<table id="roleListTable" class="table table-striped table-bordered table-condensed">
			...
			<tr>										 
				<td><a href="${ctx}/sys/role/gotoRoleEdit?editFlag=2&roleId=${role.id}">${role.name}</a></td>
				<td>${role.remarks}</td>					 
				<td nowrap>
					<a href="${ctx}/sys/role/gotoRoleEdit?editFlag=2&roleId=${role.id}">修改</a>
					<a href="javascript:roleMgr.delRole(${role.id})">删除</a>
					<a href="${ctx}/sys/role/gotoRoleEdit?editFlag=1">添加</a>
				</td>		 
			</tr>
			</c:forEach>
